<template>
    ...
    <view class="xinguan-view">
        <view class="xinguan-flex">
            <text>姓名</text>
            <input type="text" placeholder="请输入您的姓名" placeholder-class="input-style" v-model="penging_data.name" />
        </view>
        <view class="xinguan-flex">
            <text>身份证</text>
            <input type="text" placeholder="请输入您的身份证号" placeholder-class="input-style" v-model="penging_data.id_card" />
        </view>
        <view class="xinguan-flex">
            <text>手机号</text>
            <input type="number" placeholder="请输入您的手机号" placeholder-class="input-style" v-model="penging_data.phone" />
        </view>
        <view class="xinguan-flex">
            <text>现居地址</text>
        ...
        <view class="xinguan-flex">
            <text>详细地址</text>
            <input type="text" placeholder="请输入您的详细地址" v-model="penging_data.de_address"
    ...
    <view>
        ...
        <view class="period-view" v-for="(item, index) in timeData.data.lasting" :key="index">
            ...
            <view class="Week-flex">
                <block v-for="(val, $index) in item.time" :key="$index">
                    <view class="Week-style Week-Down" :class="index + '-' + $index == timeIndex ? 'checkstyle' : ''"
                        @click="ChooseTime(index + '-' + $index, item.period, val.start_time, val.end_time, val.when)">
                        <text>{{ val.start_time }}-{{ val.end_time }}</text>
                        <text>剩余{{ val.over }}</text>
                    </view>
                </block>
            </view>
        </view>
...
import type { Newapptime } from './type';
...
const getTimeData = async () => {
    let res = await RequestApi.NewappTime()
...
const changeRegion = (e) => {
    SubmitData.penging_data.address = e.detail.value.join(" ")
}
//选择疫苗人群分类
...
const changeSelector = (e: any) => {
    SubmitData.penging_data.crowd_sort = selector_data.value[e.detail.value]
}
// 时间预约
const ChooseWeek = (data: string) => {
    SubmitData.penging_data.date = data
}
// <!-- 时段 -->
let timeIndex = ref("1")
const ChooseTime = (index: string, period: string, start_time: string, end_time: string, when: number) => {
...
const submit = async () => {
    ...
    if (res.statusCode == 200) {
        uni.hideLoading()
        uni.navigateTo({ url: '/pages/service/xinguan/index' });
        SubmitData.penging_data = {
            name: "",
        id_card: "",
        phone: "",
        address: "",
