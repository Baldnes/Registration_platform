<template>
    ...
    <view class="xinguan-view">
        ...
        <view class="xinguan-flex">
            <text>手机号</text>
            <input placeholder="请输入手机号" type="number" v-model="submitData.phone" placeholder-class="input-style" />
        </view>
    </view>
    <!-- 选择套餐 -->
        ...
        <block v-for="(item, index) in hpv_combo_name" :key="index">
            ...
            <view class="combo-list">
                <text v-for="(item_a, index_a) in item.name" :key="index_a" :class="index_a == COMBO ? 'checkstyle' : ''"
                    @click="changCombo(index_a, item_a.combo_id, item_a.combo)">{{ item_a.combo }}</text>
            </view>
        </block>
        <!-- 接种时间 -->
        <block v-for="(item, index) in hpv_combo_time" :key="index">
            ...
            <view class="combo-list">
                <text v-for="(item_a, index_a) in item.name" :key="index_a" :class="index_a == TIME ? 'checkstyle' : ''"
                    @click="changTime(index_a, item_a.time_id, item_a.time)">{{ item_a.time }}</text>
            </view>
        </block>
    </view>
    <!-- 提交 -->
...
import type { Hpvcomboname, Hpvcombotime } from './type'
...
// 选中套餐名称
let COMBO = ref<number>(-1)
let combo_id = ref<string>('')
let combo_name = ref<string>('')
const changCombo = (index: number, id: string, name: string) => {
    COMBO.value = index
    combo_id.value = id
    combo_name.value = name
}
// 选中接种时间
let TIME = ref<number>(-1)
let time_id = ref<string>('')
let combo_time = ref<string>('')
...
// 请求套餐价格
let hpv_combo_price = ref<number>(0.00)
interface ResPrice{
...
let changeSelector = (event:{detail:{value:string}}):void => {
  gender.value = event.detail.value == '0' ? '男' : '女'
}
// 选择出生日期
let born_date = ref<string>('')
