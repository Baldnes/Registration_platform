<template>
  <view class="graphics-back">
    ...
    <textarea placeholder="为了更好获得医生帮助,请尽可能详细描述病情" maxlength="300" placeholder-class="textarea-color" :auto-focus="true"
  ...
  <view class="graphics-back">
    ...
    <view class="pat-image">
      <view class="upload-Image" v-for="(item, index) in submitData.ins_report" :key="index">
        ...
        <image src="/static/other/shanchu-goods.svg" mode="widthFix" @click="submitData.ins_report.splice(index, 1)">
        </image>
      </view>
      <view>
...
const choosePatient = () => {
  uni.navigateTo({
    url: "/pages/service/patient/index"
  })
}
// 监听选择的就诊人
let name = ref<string>('')
const store = myData()
store.$subscribe((mutation, state) => {//subscribe=watch
...
const subMit = async () => {
  ...
  if (res.statusCode == 200) {
    ...
    uni.showToast({ title: '提交失败', icon: 'none', duration: 1000 });
  }
}
</script>
<style>
