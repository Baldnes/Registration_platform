    ...
    <page-container :show="show" @enter="onEnter" @clickoverlay="clickoverlay">
        <view class="popups">
            ...
            <view class="table-time">
                <text v-for="(item, index) in the_time" :key="index" :class="index == selectIndex ? 'backTime' : ''"
                    @click="selectIndex = index, submitData.the_time = item">{{ item }}</text>
            </view>
            <view>
...
const subMit = async () => {
    ...
    if (res.statusCode == 200) {
        ...
        uni.redirectTo({
            url: '/pages/service/registration/index'
        })
    }
}
